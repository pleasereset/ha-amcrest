...
class EventMonitor:
    ...
    def monitor_events(self, event_codes, channel, name):
        with self._event_code_map_lock:
            ...
            _LOGGER.debug("Updating event monitor config for %s", 
                self._api._host)
            _LOGGER.debug("Now monitoring events %s for '%s' on channel %d",
                event_codes, name, channel)

    def _get_channels_for_event(self, event_info):
        """Returns a set of channels for which the event occured"""
    ...
    def _parse_event_channel_info(self, api_result):
        """Turns the result of an amcrest event_channels_happened API call
        into a set of channel indices"""
    ...
    def _parse_event_info(self, raw_event):
        """Turns raw event strings from the amcrest lib into a key-value 
        dictionary"""
        ...
        for event_kv in event_key_values:
            try:
                ...
                event_info[clean_str(key)] = clean_str(value)
            except ValueError:
                _LOGGER.warn("Received invalid/truncated event information, " \
                    + "best-effort parsing: %s", raw_event)
        return event_info
    ...
    def _worker_process_event(self, raw_event_info, active_events):
        ...
        if EVENT_KEY_ACTION not in event_info:
            _LOGGER.error(
                "Received an event from the camera without a " \
        ...
        if action == EVENT_ACTION_START:
            ...
            channels_occured = self._get_channels_for_event(
                event_info
            )
            _LOGGER.debug(
                "Event %s started on channels %s, %s subscribed",
                code, channels_occured, channels_subscribed
            )
            channels_of_interest = \
                channels_occured.intersection(channels_subscribed)
            ...
            if code in active_events:
                ...
                for channel in active_events[code]:
                    _LOGGER.debug("Event %s stopped on channel %d", 
                        code, channel)
    ...
    def _worker_thread_main(self):
        _LOGGER.debug(
            "Event monitor worker thread starting for: %s", self._api._host
        )
        ...
        while True:
            ...
            try:
                ...
                _LOGGER.warning(
                    "Error while processing camera events: %r", error
                )
